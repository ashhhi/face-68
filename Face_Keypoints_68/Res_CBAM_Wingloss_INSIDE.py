...
checkpoint_save_path = 'CKPT/Res_CBAM_Wingloss_INSIDE_CKPT\Resnet.ckpt'
...
if isTrain == True:
    ...
    model.summary()
    # Save_history('log/Res_CBAM_INSIDE.csv', model.history)
    ...
    for i in range(x_train.shape[0]):
        ...
        for k in range(len(pts)):
            cv.circle(img, (int(pts[k][0]), int(pts[k][1])), 1, (0, 255, 0))
        # cv.imshow("pred_landmark", img)
        # cv.waitKey()
        cv.imwrite("pred_pic/"+ str(i)+".jpg",img)
        print(i)
